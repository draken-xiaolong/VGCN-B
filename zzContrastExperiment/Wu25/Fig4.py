# -*- coding: utf-8 -*-
"""
Wu25/Fig4.py - 噪声攻击的NC评估（可逆水印，复用 Wu25 的提取与攻击管线）
- 攻击：ratio 0.1, 0.3, 0.5 × strength 0.2, 0.6, 1.0, 1.4
- 数据：使用 `embed/` 下 6 个含水印矢量文件
- 结果：输出到 `NC-Results/Fig4/` 下的 CSV、XLSX、PNG
- 结构与 Tan24/zNC-Test 的 Fig4 保持一致的层次化展示
"""
...
# 与 Tan24 对齐：强度 0.4/0.6/0.8；比例 10/30/50/70/90%
STRENGTHS = [0.4, 0.6, 0.8]
RATIOS = [i / 100 for i in [10, 30, 50, 70, 90]]


def run_evaluation():
    print('=== Wu25 Fig4：噪声攻击 NC 评估 ===')
    ...
    rows = []
    nc_tensor = np.zeros((len(FILE_NAMES), len(STRENGTHS), len(RATIOS)))
        ...
        for s_idx, strength in enumerate(STRENGTHS):
            for r_idx, ratio in enumerate(RATIOS):
                try:
                    ...
                    print(f'  s={strength:.1f}, r={ratio:.1f} -> NC={nc_value:.4f}, BER={ber_value:.4f}')
