...
def NC(original_watermark, extract_watermark):
    ...
    # 转换为float64避免溢出，并确保是numpy数组
    original_watermark = np.array(original_watermark, dtype=np.float64)
    extract_watermark = np.array(extract_watermark, dtype=np.float64)
    
    # 获取图像尺寸
    m, n = original_watermark.shape
    
    # 按照NC.m的逻辑计算NC值，使用float64避免溢出
    fenzi = np.float64(0)   # 分子
    fenmu1 = np.float64(0)  # 分母1：mark_get的平方和
    fenmu2 = np.float64(0)  # 分母2：mark_prime的平方和
    ...
    # 添加小的epsilon避免除零错误
    epsilon = 1e-10
    denominator = np.sqrt(fenmu1 * fenmu2 + epsilon)
    N = fenzi / denominator
