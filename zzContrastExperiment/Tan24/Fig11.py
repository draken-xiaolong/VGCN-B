...
8) Fig8 顺时针旋转 180°
9) Fig9 Y轴镜像翻转
10) Fig10 顺序：反转顶点顺序，再反转对象顺序

目标：在保持 Tan24 零水印方法不变的前提下，采用与 zNC-Test/Fig11.py 一致的组合攻击总体流程，
以 Tan24 数据为输入，输出到 `Tan24/NC-Results/Fig11`（CSV/XLSX/PNG）。
"""
...
def generate_compound_attacks(shp_files: List[Path]) -> Dict[str, Dict[str, Path]]:
    """生成 10 种固定组合攻击（输出 Shapefile）。

    返回: {base: {combo_key: out_path}}
    """
    print('[Step2] 生成组合攻击 ->', DIR_ATTACKED)
        ...
        for key, _label in COMBOS:
            try:
                ...
                if key == 'del_vertices_10pct':
                    ...
                    # 反转顶点顺序 → 反转对象顺序
                    attacked['geometry'] = attacked['geometry'].apply(lambda geom: add_vertices_to_geom(geom, 0, 0))  # no-op to keep dtype
...
def evaluate_nc(attacked_map: Dict[str, Dict[str, Path]]):
    ...
    plt.xlabel('组合攻击')
    plt.ylabel('NC')
    plt.title('Fig11：Tan24 组合攻击(10种) 的NC鲁棒性')
...
def main():
    print('=== Fig11：Tan24 组合攻击鲁棒性测试（与 Fig11 逻辑一致）===')
    ...
    attacked_map = generate_compound_attacks(inputs)
