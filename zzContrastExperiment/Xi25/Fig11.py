# -*- coding: utf-8 -*-
"""
Xi25/Fig11.py - 组合攻击（10种单例组合）的NC评估
...
def run_evaluation():
	print('=== Xi25 Fig11：组合攻击(10种单例) NC 评估 ===')
	missing = []
	...
	for file_idx, (vector_file, file_name) in enumerate(zip(USE_FILES, USE_NAMES)):
		...
		for k_idx, key in enumerate(order_keys):
			try:
				...
				nc_value = _xi25_extract_nc(out_path, SCRIPT_DIR / WATERMARK)
	...
	plt.xlabel('组合攻击')
	plt.ylabel('NC')
	plt.title('组合攻击(10种) 的NC鲁棒性（Xi25/Fig11）')
