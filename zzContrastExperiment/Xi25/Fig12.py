# -*- coding: utf-8 -*-
"""
Xi25/Fig12.py - 复合攻击（一次性按 Fig1→Fig10 顺序依次执行）的NC评估
- 步骤与 Tan24(Fig12) 一致，但提取采用 Xi25 提取器
...
def generate_compound_seq(src_path: Path) -> Path:
	...
	_ensure_crs(gdf, src_path, gpd.read_file(str(src_path)))
	out_path = subdir / 'compound_seq_all.shp'
	gdf.to_file(str(out_path), driver='ESRI Shapefile')
	return out_path


def _xi25_extract_nc(attacked_path: Path, watermark_path: Path, file_name: str) -> float:
...
def run_evaluation():
	print('=== Xi25 Fig12：复合(顺序) 攻击 NC 评估 ===')
	missing = []
	...
	# Xi25 不支持点数据：这些文件原本是点数据，虽然在嵌入过程中被转换成了线数据，
	# 但为了公平对比，应该跳过它们
	POINT_DATA_FILES = {'HYDP', 'gis_osm_natural_free_1'}
	
	# 仅选择非点数据
	selected: List[tuple[Path, str]] = []
	for vector_file, file_name in zip(VECTOR_FILES, FILE_NAMES):
		# 检查是否是原本的点数据
		if file_name in POINT_DATA_FILES:
			print(f'跳过点数据（原本是点矢量）: {file_name}')
			continue
	...
	for vector_file, file_name in zip(USE_FILES, USE_NAMES):
		print(f'处理: {file_name}')
		out_path = generate_compound_seq(Path(str(vector_file)))
		try:
			nc_value = _xi25_extract_nc(out_path, SCRIPT_DIR / WATERMARK, file_name)
	...
	plt.ylabel('NC')
	plt.title('复合攻击(一次性顺序 Fig1→Fig10) 的NC鲁棒性（Xi25/Fig12）')
	plt.legend()
