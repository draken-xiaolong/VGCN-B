...
# -*- coding: utf-8 -*-
"""
Fig8：旋转攻击鲁棒性测试脚本（顺时针角度：45, 90, 135, 180, 225, 270, 315, 360）

步骤与 Fig1 对齐，输出目录隔离在 `rotate` 子目录。
"""
...
DIR_VECTOR_GEOJSON_ATTACKED = SCRIPT_DIR / 'vector-data-geojson-attacked' / 'rotate'
DIR_GRAPH = SCRIPT_DIR / 'vector-data-geojson-attacked-graph'
DIR_GRAPH_ORIGINAL = DIR_GRAPH / 'Original'
DIR_GRAPH_ATTACKED = DIR_GRAPH / 'Attacked' / 'rotate'
...
def step3_generate_rotate_attacks(original_geojsons: List[Path]) -> Dict[str, Dict[int, Path]]:
    print('[Step3] 生成旋转攻击 ->', DIR_VECTOR_GEOJSON_ATTACKED)
    if gpd is None:
    ...
    for src in original_geojsons:
        ...
        for deg in ROTATE_DEGREES:
            ...
            out_path = subdir / f'rotate_{deg}deg.geojson'
            try:
        ...
        for pkl in sorted(base_dir_path.glob('*_graph.pkl')):
            try:
                ...
                for d in ROTATE_DEGREES:
                    if f'rotate_{d}deg' in name: deg = d; break
                rows.append({'base': base, 'deg': deg if deg is not None else -1, 'nc': nc})
            except Exception as exc:
...
def main():
    ...
    if existing['step3']:
        ...
        attacked_map: Dict[str, Dict[int, Path]] = {}
        for attacked_dir in DIR_VECTOR_GEOJSON_ATTACKED.glob('*'):
            if attacked_dir.is_dir():
                base = attacked_dir.name; attacked_map[base] = {}
                for d in ROTATE_DEGREES:
                    geojson_file = attacked_dir / f'rotate_{d}deg.geojson'
                    if geojson_file.exists(): attacked_map[base][d] = geojson_file
    ...
    if existing['step2']:
        ...
        original_geojsons = [p for p in DIR_VECTOR_GEOJSON.glob('*.geojson') if not p.name.startswith('._')]
        attacked_map = step3_generate_rotate_attacks(original_geojsons)
        step4_convert_to_graph(original_geojsons, attacked_map); step5_generate_zero_watermark(); step6_evaluate_nc(); print('=== 完成 ==='); return
    print('\n[从头开始] 运行所有步骤...')
